...
class CMDParser(object):
    def __init__(self):
        ...
        aslib = self._arg_parser.add_argument_group("Reading from ASlib Format")
        aslib.add_argument(
            "-s",
            "--scenario",
            default=None,
            help=
            "directory with ASlib scenario files (required if not using --load or csv input files"
        )
        ...
        csv.add_argument(
            "--performance_test_csv",
            default=None,
            help=
            "performance *test* data in csv table (column: algorithm, row: instance, delimeter: ,)"
        )
        csv.add_argument(
            "--feature_test_csv",
            default=None,
            help=
            "instance *test* features data in csv table (column: features, row: instance, delimeter: ,)"
        )
        csv.add_argument(
            "--cv_csv",
            default=None,
            help=
            "cross validation splits in csv table (column: split ID, row: instance, delimeter: ,)"
        )
        csv.add_argument(
            "--objective",
            default="solution_quality",
            choices=["runtime", "solution_quality"],
            help=
            "Are the objective values in the performance data runtimes or an arbitrary solution quality (or cost) value"
        )
        csv.add_argument(
            "--runtime_cutoff",
            default=None,
        ...
        csv.add_argument(
            ...
            action="store_true",
            help=
            "Set this parameter to indicate maximization of the performance metric (default: minimization)"
        )
        ...
        opt.add_argument("--smac_seed", default=42, type=int, help="Seed passed to SMAC")
        opt.add_argument("-p", "--pcs", default=None, help="pcs file to be read")
        opt.add_argument("--output_dir", default=None, help="output directory of SMAC")
        opt.add_argument(
            "--runcount_limit",
            type=int,
            default=42,
        ...
        opt.add_argument(
            ...
            type=str,
            help=
            "feature vector to predict algorithm to use -- has to be used in combination with --load"
        )
