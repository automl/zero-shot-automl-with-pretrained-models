...
class Stats(object):
    ...
    def show(self, remove_unsolvable: bool = True):
        ...
        if self.runtime_cutoff:
            ...
            self.logger.info(
                "Presolved during feature computation: %d / %d" % (self.presolved_feats, n_samples)
            )
...
class Validator(object):
    ...
    def validate_runtime(
        ...
        feature_stati = test_scenario.feature_runstatus_data[test_scenario.used_feature_groups]
        ...
        for inst, schedule in schedules.items():
            ...
            for algo, budget in schedule:
                ...
                if time <= budget and used_time <= test_scenario.algorithm_cutoff_time and test_scenario.runstatus_data[
                    ...
                    stat.solved += 1
                    self.logger.debug(
                        "Solved by %s (budget: %f -- required to solve: %f)" % (algo, budget, time)
                    )
                    break
