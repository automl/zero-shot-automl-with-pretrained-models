...
from autofolio.feature_preprocessing.pca_task2vec import PCAWrapper as PCAWrapper_task2vec
...
class AutoFolio(object):
    def __init__(self, random_seed: int = 5456):
        ''' Constructor
    ...
    def run_cli(self, args):
        ...
        self.task2vec = True if 'task2vec' in args['feature_csv'] else False

        self._root_logger.setLevel(args_.verbose)

        print("CV CSV", args_.cv_csv)
        if args_.load:
            ...
            print("TUNED PIPELINE CONFIG", config)
    ...
    def _outer_cv(
        ...
        for cv_fold in outer_cv_folds:
            ...
            config = self.get_tuned_config(
                outer_training, autofolio_config=autofolio_config, seed=smac_seed
            )

            # Use `AutoFolio.run_fold()’ to get the performance on the outer split
            stats, fit, schedule = self.run_fold(config, scenario, cv_fold, return_fit=True)
            ...
            if out_template is not None:
                ...
                self._save_model(
                    model_fn, scenario, feature_pre_pipeline, pre_solver, selector, config
                )
    ...
    def get_cs(self, scenario: ASlibScenario, autofolio_config: dict = None):
        ...
        allowed_feature_groups = autofolio_config.get(
            "allowed_feature_groups", scenario.feature_steps
        )
        ...
        for fs in allowed_feature_groups:
            ...
            fs_param = CategoricalHyperparameter(
                name="fgroup_%s" % (fs), choices=choices, default_value=default
            )
        ...
        if autofolio_config.get("pca", True):
            if self.task2vec:
                PCAWrapper_task2vec.add_params(self.cs)
            else:
        ...
        if autofolio_config.get("classifier"):
            ...
            cls_choices = ["RandomForest"]
            cls_def = "RandomForest"
        classifier = CategoricalHyperparameter(
            "classifier", choices=cls_choices, default_value=cls_def
        )
        ...
        if autofolio_config.get("selector"):
            ...
            sel_choices = ["MultiClassifier", "JointRegressor"]
            sel_def = "MultiClassifier"

        selector = CategoricalHyperparameter("selector", choices=sel_choices, default_value=sel_def)
        self.cs.add_hyperparameter(selector)
        MultiClassifier.add_params(self.cs)
        JointRegression.add_params(self.cs)
    ...
    def get_tuned_config(
        ...
        smac = SMAC(scenario=ac_scenario, tae_runner=taf, rng=np.random.RandomState(seed))
    ...
    def run_cv(self, config: Configuration, scenario: ASlibScenario, folds: int = 10):
        ...
        try:
            ...
            for i in range(1, folds + 1):
                ...
                stats = self.run_fold(config=config, scenario=scenario, fold=i)
    ...
    def run_fold(
        ...
        schedules = self.predict(test_scenario, config, feature_pre_pipeline, pre_solver, selector)
    ...
    def fit(self, scenario: ASlibScenario, config: Configuration):
        ...
        scenario, feature_pre_pipeline = self.fit_transform_feature_preprocessing(scenario, config)
    ...
    def predict(
        ...
        if pre_solving_schedule:
            return dict(
                (inst, pre_solving_schedule.get(inst, []) + schedule)
                for inst, schedule in pred_schedules.items()
            )
        else:
