...
class RandomForestRegressor(object):
    ...
    def add_params(cs: ConfigurationSpace):
        ...
        try:
            ...
            cs.add_condition(cond)
            cond = InCondition(child=max_depth, parent=regressor, values=["RandomForestRegressor"])
            cs.add_condition(cond)
            cond = InCondition(
            ...
            cs.add_condition(cond)
            cond = InCondition(child=bootstrap, parent=regressor, values=["RandomForestRegressor"])
            cs.add_condition(cond)
