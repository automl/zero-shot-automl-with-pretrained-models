...
def construct_model_path(save_path, config_seed, split_type, loo, cv, mode, weighted, weigh_fn, sparsity, use_meta):
    ...
    if split_type!="loo":
        model_path = os.path.join(save_path, f"{'weighted-' if weighted else ''}{mode}{extra}", str(config_seed), str(cv))
    else:
        model_path = os.path.join(save_path+"-cvplusloo", f"{'weighted-' if weighted else ''}{mode}{extra}", str(config_seed), str(loo), str(cv))
