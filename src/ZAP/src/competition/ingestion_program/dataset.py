    ...
    """
        ...
        for i in range(self.metadata_.get_bundle_size()):
            if self.metadata_.is_sparse(i):
                sequence_features[self._feature_key(i, "sparse_col_index")] = tf.VarLenFeature(
                    tf.int64
                )
                sequence_features[self._feature_key(i, "sparse_row_index")] = tf.VarLenFeature(
                    tf.int64
                )
                ...
                sequence_features[self._feature_key(i, "dense_input")] = tf.FixedLenSequenceFeature(
        ...
        for i in range(self.metadata_.get_bundle_size()):
            ...
            if key_compressed in features:
                ...
                images = tf.map_fn(decompress_image_func, compressed_images, dtype=tf.float32)
