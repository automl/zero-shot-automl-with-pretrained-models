...
def write_timestamp(output_dir, predict_idx, timestamp):
    ...
    logger.debug(
        "Wrote timestamp {} to 'start.txt' for predition {}.".format(timestamp, predict_idx)
    )
...
def ingestion_fn(
    ...
    write_start_file(
        output_dir, start_time=start, time_budget=time_budget, task_name=basename.split(".")[0]
    )
    ...
    try:
        ...
        while not (use_done_training_api and M.done_training):
            ...
            if Y_pred is None:  # Stop train/predict process if Y_pred is None
                logger.info(
                    "The method model.test returned `None`. " + "Stop train/predict process."
                )
                break
