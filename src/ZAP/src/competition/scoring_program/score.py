...
def plot_learning_curve(
  ...
  """
    ...
    if method == "step":
        ...
        raise ValueError("The `method` variable should be one of " + "['step', 'trapez']!")
...
class LearningCurve(object):
  ...
  """
    ...
    def __add__(self, lc):
        ...
        participant_name = (
            self.participant_name if self.participant_name == lc.participant_name else None
        )
        algorithm_name = self.algorithm_name if self.algorithm_name == lc.algorithm_name else None
...
class Evaluator(object):
    ...
    """
        prediction_files = ls(os.path.join(self.prediction_dir, self.prediction_filename_pattern()))
        ...
        with open(filepath, mode) as html_file:
            ...
            for image_path in image_paths:
                with open(image_path, "rb") as image_file:
                    ...
                    s = '<img src="data:image/png;charset=utf-8;base64,{}"/>'.format(encoded_string)
