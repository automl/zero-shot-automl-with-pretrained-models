...
def mvmean(R, axis=0):
    ...
    average = lambda x: reduce(
        lambda i, j: (0, (j[0] / (j[0] + 1.0)) * i[1] + (1.0 / (j[0] + 1)) * j[1]), enumerate(x)
    )[1]
    R = np.array(R)
    if len(R.shape) == 1:
        return average(R)
...
def bac_metric(solution, prediction, task="binary.classification"):
    ...
    if (task != "multiclass.classification") or (label_num == 1):
        ...
        bac = tpr
        base_bac = 1.0 / label_num  # random predictions for multiclass case
...
def pac_metric(solution, prediction, task="binary.classification"):
    ...
    pos_num = 1.0 * sum(solution)  # float conversion!
...
if __name__ == "__main__":
    ...
    p = len(sol1)
    Pred.append(np.array([sum(sol1) * 1.0 / p] * p))
    Sol.append(sol1)
    ...
    Pred.append(
        np.array([[0.75, 0.25, 0.0], [0.75, 0.25, 0.0], [0.75, 0.25, 0.0], [0.75, 0.25, 0.0]])
    )
    Sol.append(sol2)
    ...
    comment.append("Three classes predict prior")
    Pred.append(
        np.array([[0.25, 0.25, 0.5], [0.25, 0.25, 0.5], [0.25, 0.25, 0.5], [0.25, 0.25, 0.5]])
    )
    Sol.append(sol4)
