...
def load_pretrained_weights(model, model_name, model_dir=None, advprop=False):
    ...
    state_dict = model_zoo.load_url(url_map_[model_name], model_dir=model_dir)
    state_dict.pop('_fc.weight')
    state_dict.pop('_fc.bias')
    res = model.load_state_dict(state_dict, strict=False)
    #assert set(res.missing_keys) == set(['stem.0.mean', 'stem.0.std', '_fc.weight', '_fc.bias']), 'issue loading pretrained weights'
