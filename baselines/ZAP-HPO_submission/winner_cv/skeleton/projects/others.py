...
def get_tf_resize(height, width, times=1, min_value=0.0, max_value=1.0):
    def preprocessor(tensor):
        ...
        if width == in_width and height == in_height:
            ...
            tensor = tf.image.resize_images(
                tensor, (height, width), method=tf.image.ResizeMethod.BICUBIC
            )
        ...
        if times != in_times or times > 1:
            ...
            tensor = tf.image.resize_images(
                tensor, (times, height * width), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR
            )
