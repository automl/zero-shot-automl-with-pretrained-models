...
from tester import ModelPredictor
import config2vector
from loader import list_of_metafeatures, bool_hps, categorical_hps, numerical_hps, apply_log
...
class LogicModel(Model):
    def __init__(self, metadata, session=None, model_config=None):
        ...
        metadata_path = Path(__file__).parents[5]/"data/meta_dataset"

        config_dir = metadata_path / "configs" / "kakaobrain_optimized_per_icgen_augmentation"
        config_paths = config2vector.get_config_paths(config_dir)
        print(f"Number of configurations: {len(config_paths)}")

        hps = numerical_hps+bool_hps+categorical_hps
        config_X = []
        for path in config_paths:
            hp_dict = config2vector.create_searchspace(path)
            config_X.append([hp_dict[k] for k in hps])
        
        columns = list_of_metafeatures+hps
        ...
        print("The prediction indices are: ", predictions)
